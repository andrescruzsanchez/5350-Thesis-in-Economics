...
school_data <- school_data %>%
  ...
  # CHANGE LANGUAGE HERE
  # "English" (eng), "Swedish" (sv), or "Mathematics" (ma)
  rename(share_students_F = share_students_F_ma) %>%
...
box_plot <- ggplot(
  ...
  labs(
    title = "Boxplot of Share of Students with F - Unbalanced",
...
output_file <- file.path(path_output, paste0("Box_plot_unbalanced", language, ".png"))
ggsave(output_file, box_plot, bg = "transparent", width = 4, height = 3, dpi = 1800)
...
# Save the plot using a dynamic file name
output_file <- file.path(path_output, paste0("Box_plot_balanced", language, ".png"))
ggsave(output_file, box_plot, bg = "transparent", width = 4, height = 3, dpi = 1800)
...
# Save the table to a dynamic file name
output_file <- file.path(path_output, paste0("Static_DiD_output_", language, ".html"))
