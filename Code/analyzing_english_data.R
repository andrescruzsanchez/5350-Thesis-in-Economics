...
school_data <- read_excel("school_data.xlsx", sheet = "English")
...
dynamic_DiD <- lm(share_students_F_eng ~ academic_year + treatment_group + academic_year:treatment_group + 
...
event_study <- lm(share_students_F_eng ~ (lead_5 + lead_4 + lead_3 + lead_2 + lag_0)*treatment_group +
                               share_foreign_background + share_postsecondary_parents + 
...
# Use file.path() to ensure correct formatting of the file path
output_file <- file.path(path_output, "regression_output_english_robust_standard_errors.html")
